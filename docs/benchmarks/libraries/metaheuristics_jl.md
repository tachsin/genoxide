...
- **Stops and restarts** (rule 2.2), `options`, `algorithm_kwargs` and `run_restarting` (lines 210-250):
...
- **Early stop** (rule 5.3): a solver whose first 3 seeds all hit the 60 s cap runs no more seeds (`EARLY_SEEDS`, line 418, and `main`).
...
**Methods** (`nqueens_solvers`, lines 330-355): the guide: "Permutation-based: Use GA with OrderCrossover or BRKGA".
...
**The front** (rule 7.2): the non-dominated part of the final population (`status.population`), whose size is the scenario's (92 or 100), with the objective values the library computed; after a restart, the last attempt's.

**Keeping going:** `RobustConvergence` would restart an attempt; it never triggered here.
...
| DTLZ2 | sms_emoa | 3 | 0.7344 (0.7370, 0.7339) | 5,980 | 3 |
