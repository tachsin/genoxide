...
| `.memetic(parents, neighbors)` | no | off | 1 ≤ parents ≤ population size, neighbors ≥ 1: the best parents each try neighbors made by the mutation operator, and take the best one if not worse (Lamarckian) |
